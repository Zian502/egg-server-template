...
import { ModelConvertor } from './utils/ModelConvertor';
...
export class UserRepository extends AbstractRepository {
  async saveUser(user: UserEntity): Promise<void> {
    if (user.userId) {
      const model = await UserModel.findOne({ id: user.userId });
      if (!model) return;
  ...
  async findTokenByTokenKey(tokenKey: string): Promise<TokenEntity | any> {
